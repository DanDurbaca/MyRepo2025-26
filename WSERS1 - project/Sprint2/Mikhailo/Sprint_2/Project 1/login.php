...
function checkLogin($username, $password, $secret) {
    ...
    while (!feof($file)) {
        ...
        if (count($userData) < 4) continue;

        $fileUser   = trim($userData[0]);
        $fileEmail  = trim($userData[1]);
        $filePass   = trim($userData[2]);
        $fileSecret = trim($userData[3]);
        $fileAdmin =  trim($userData[4]);

        if ($fileUser === $username) {
            fclose($file);

            if (!password_verify($password, $filePass)) return "wrong_password";
            if (!password_verify($secret, $fileSecret)) return "wrong_secret";
