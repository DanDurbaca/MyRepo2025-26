    ...
    <div id="nav-list">
      <ul>
        ...
        <?php if ($_SESSION["isLogged"]) { ?>
          ...
          <li id="<?= $activePage === "/order-history/index.php" ? "active-page" : "" ?>"><a href="<?= $opts["pages"]["order-history"] ?? $default["pages"]["order-history"] ?>">Order History</a></li>
        <?php } ?>
    ...
    <?php } else { ?>
      <?php if (!$_SESSION["isAdmin"]) { ?>
        <div id="cart">
          ...
          <div id="cart-quantity"><p><?= empty($_SESSION["Cart"]) ? 0 : count($_SESSION["Cart"]) ?></p></div>
        </div>
      <?php } ?>

      <div id="profile">
        <div id="profile-picture"><img src="<?= $opts["profile-pic"] ?? $default["profile-pic"] ?>" alt="profile-pic" /></div>
        <div id="profile-name"><?= $_SESSION["username"] ?></div>
      </div>
    <?php }?>
