...
if(isset($_POST["Checkout"])){
    $sqlQuery=$connection->prepare("insert into orders(statusEN, statusDE, username) values('Pending', 'Ausstehend',?);");
    $sqlQuery->bind_param("s", $_SESSION["Userlogged"]);
    $sqlQuery->execute();
    $orderid=$connection->insert_id;
    foreach ($_SESSION["Cart"] as $itemID => $itemQuantity) {
        $sqlQuery = $connection->prepare("INSERT INTO boughtitem(quantity, orderid, ProductNameEN) values(?,?,?);");
        $sqlQuery->bind_param("iis", $itemQuantity, $orderid, $itemID);
        $sqlQuery->execute();
        unset($_SESSION["Cart"]);
        unset($_POST["Checkout"]);
    }
    ?><script>alert("Purchase successful")</script><?php
    header("Refresh:0; url=index.php?lang=".$language);
}
if(isset($_POST["orderToSend"])){
    $sqlQuery=$connection->prepare("update orders set statusEN = 'Delivered', statusDE='Geliefert' where orderid=?;");
