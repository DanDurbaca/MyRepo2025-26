...
if (isset($_POST["Username"], $_POST["psw"], $_POST["pswAgain"])) {
    ...
    if ($_POST["psw"] == $_POST["pswAgain"] && (!checkIfUserExists($_POST["Username"]))) {
        ...
        $sqlInsert = $connection -> prepare("INSERT INTO Clients (Username, userPassword, userTypes) VALUES (?,?,?)");
        $hashedPassword = password_hash($_POST["psw"],PASSWORD_DEFAULT);
        $defaultUserType = "regular_customer";
        $sqlInsert -> bind_param("sss",$_POST["Username"], $hashedPassword, $defaultUserType);
        $sqlInsert -> execute(); //hope this works :))
