...
// Ensure critical order tables exist to avoid fatal errors when the DB hasn't been fully imported.
function ensureOrderTables()
{
    $db = getDB();

    // helper to check for a table in current schema
    $checkTable = function ($tableName) use ($db) {
        $st = $db->prepare("SELECT COUNT(*) as cnt FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = ?");
        if (!$st) return 0;
    ...
    $hasOrders = $checkTable('Orders');
    if ($hasOrders > 0) {
        return; // already present
    }

    // Ensure Clients and Products exist; we need them for FK references
    $hasClients = $checkTable('Clients');
    $hasProducts = $checkTable('Products');

    if ($hasClients === 0 || $hasProducts === 0) {
        // Can't safely create Orders table without Clients and Products; log and return.
        error_log("ensureOrderTables: Clients or Products table missing; skipping automatic Orders creation.");
        return;
    }
    ...
    if (!$db->query($sql1)) {
        error_log('ensureOrderTables: failed to create Orders: ' . $db->error);
        return;
    }

    // Create BoughtProducts table
    $sql2 = "CREATE TABLE IF NOT EXISTS BoughtProducts(
        id INT PRIMARY KEY AUTO_INCREMENT,
        orderId INT,
        productId INT,
        quantity INT,
    ...
    if (!$db->query($sql2)) {
        error_log('ensureOrderTables: failed to create BoughtProducts: ' . $db->error);
        return;
    }

    error_log('ensureOrderTables: Orders and BoughtProducts tables created automatically.');
}

// Try to create missing order tables if necessary (non-intrusive)
ensureOrderTables();
...
function NavigationBar($page)
{
    ...
    // Allow logged in users (regular and admin) to access their orders page
    if (isset($_SESSION['logged_in_user'])) {
        $navigationsBarLinks["Orders"] = "orders.php";
    }
