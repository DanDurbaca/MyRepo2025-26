            ...
            <div class="left card">
                ...
                if ($_SESSION["ADMIN"] == 1) {
                    ...
                    if (isset($_POST["add_product"])) {
                        if (
                            !isset($_POST["pdc_name"], $_POST["pdc_price"], $_POST["pdc_description"], $_POST["pdc_namePT"], $_POST["pdc_descriptionPT"]) ||
                                ...
                                if (filesize($file['tmp_name']) <= MAX_SIZE) {
                                    ...
                                    if (isset($allowedFiles[$mime_type])) {
                                        ...
                                        if (move_uploaded_file($file['tmp_name'], $destination)) {
                                            ...
                                            $connection = new mysqli("localhost", "root", "", "webshopdb");
                                            $sqlQuery = $connection->prepare("INSERT INTO products(productNameEN, ImageLink, productPrice, descriptionEN, descriptionPT, productNamePT) values(?,?,?,?,?,?);");
                                            $sqlQuery->bind_param("ssssss", $_POST["pdc_name"], $pdc_link, $_POST["pdc_price"], $_POST["pdc_description"], $_POST["pdc_descriptionPT"], $_POST["pdc_namePT"]);
                                            $sqlQuery->execute();
                ...
                <div id="userBox">
                    <table class="cartTable">
                        ...
                        <tbody>
                            <?php
                            $sqlselect = $connection->prepare("SELECT * FROM clients join orders using (clientID)");
                            $sqlselect->execute();
                            $result = $sqlselect->get_result();


                            if (isset($_POST["statusChange"])) {
                                $statuschanging = $_POST["statusChange"];
                                $sqlchange = $connection->prepare("UPDATE orders SET OrderStatus = 'Confirmed' WHERE OrderID = ?; ");
                                $sqlchange->bind_param("i", $statuschanging);
                                $sqlchange->execute();
                                header("Refresh:0; url=admin.php?lang=" . $language);
                            }
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td>
                                        <table><?php
                                                $sqlQuery = $connection->prepare("SELECT p.productNameEN, bi.quantity FROM boghtitem bi join products p on bi.productId=p.productId where orderId=?");
                                                $sqlQuery->bind_param("i", $row["OrderID"]);
                                                $sqlQuery->execute();
                                                $subresult = $sqlQuery->get_result();
                                                while ($subRow = $subresult->fetch_assoc()) {
                                                ?>
                                                <tr>
                                                    <td><?= $subRow[($language == "EN") ? "productNameEN" : "productNamePT"] ?></td>
                                    ...
                                    <td>
                                        <form method="POST" style="display: inline;">
                                            <button type="submit" name="statusChange" value="<?= $row["OrderID"] ?>">
                                                <?= ($language == "EN") ? "change" : "Mudar" ?>
                                            </button>
                                        </form>
                                    </td>
                                </tr>
