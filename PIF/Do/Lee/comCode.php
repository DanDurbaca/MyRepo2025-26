...
// CRITICAL: Prevent auto-login from stale sessions
// Always initialize these variables to ensure they're never just "set" without a value
...
function NavigationBar($status)
{
    ...
    <div class="nav-inner center">
      ...
      <ul class="nav-links">
        ...
        <?php if (!empty($_SESSION["UserLoggedIn"]) || !empty($_SESSION["adminLoggedIn"])) { ?>
          <li><a href="./stationPage.php" <?php if ($status == "game") { echo 'class="active"'; } ?>><?= htmlspecialchars($text["navSta"]) ?></a></li>
        <?php } ?>
        ...
        <?php } ?>
        <li><a href="./register.php" <?php if ($status == "register") { echo 'class="active"'; } ?>><?php
          if (!empty($_SESSION["UserLoggedIn"]) && !empty($_SESSION["userName"])) {
            echo htmlspecialchars($_SESSION["userName"]);
          } else if (!empty($_SESSION["adminLoggedIn"])) {
            echo "Admin";
          } else {
  ...
  <script>
    (function(){
      ...
      } catch(e){ /* fail silently on older pages */ }
    })();
  </script>
